...
# =======================================================================返回函数，闭包
# 将函数名作为参数的返回，函数外部接收到函数名之后再调用，才会得到结果
def f(l):
	def f1():					# 函数的内部函数f1引用了外部函数的变量，闭包
		sum = 0
		for i in l:
			if type(i) == int:
				sum = sum + i
...
l = [1, 2, 3, 'a', 4]
r = f(l)		# 返回一个函数
print(r)		# <function f.<locals>.f1 at 0x7f54665210d0>
print(r())		# 真正调用


# 返回的函数内部如果有引用可能变化的变量，该变量的值取决于外部函数返回时的该变量的值，而不是函数定义时该变量的值
# 因为在外部函数返回时内部的函数才执行
def count():
	fs = []
	for i in range(1, 4):
		def f():
			return i*i
		fs.append(f)
	return fs			# 返回三个元素的list

f1, f2, f3 = count()	# 接收返回的三个元素
print(f1())				# 返回9，而不是1，f1定义时i的值为1，但是当f1被返回时，i的值是3

# 为避免上述的情况，应该避免在闭包中的内部函数引用任何循环变量，或者后续会发生变化的变量

# 如果要引用循环变量，可以在里面定义一个函数，将循环变量绑定到该函数的参数上，已经绑定到参数的变量就不会再改变
def count():
    def f(j):
        def g():
            return j*j
    ...
    fs = []
    for i in range(1, 4):
        fs.append(f(i)) # f(i)立刻被执行，因此i的当前值被传入f()，返回g函数，g函数的中j取的是绑定到f函数的参数
    return fs
f1, f2, f3 = count()
print(f1())				# 1

# ===========================================================================匿名函数
# lambda param : 要返回的表达式
# lambda匿名函数只能有一个表达式
l = [1, 2, 3, 4, 5]
print(list(map(lambda x: x * x, l)))	# [1, 4, 9, 16, 25]
f = lambda x: x * x		# 将匿名函数的地址赋值变量
print(f(2))				# 4，通过变量调用函数